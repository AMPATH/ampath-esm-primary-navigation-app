/** @module @category Utility */ import { useCallback, useMemo } from "react";
import useSWR from "swr";
import { openmrsFetch } from "@openmrs/esm-api";
import useAbortController from "./useAbortController.js";
function getUrl(key, url) {
    if (url) {
    ...
    throw new Error(`When using useOpenmrsSWR with a key that is not a string, you must provide a url() function that converts the key to a valid url. The key for this hook is ${key}.`);
}
/**
 * @beta
 *
 ...
 */ export function useOpenmrsSWR(key, options = {}) {
    const { abortController, fetchInit, url, swrConfig } = options;
    const ac = useAbortController();
    const abortSignal = useMemo(()=>fetchInit?.signal ?? abortController?.signal ?? ac.signal, [
