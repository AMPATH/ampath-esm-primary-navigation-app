/** @module @category UI */ import { makeUrl } from "@openmrs/esm-api";
import { useServerPagination } from "./useOpenmrsPagination.js";
/**
 ...
 * @returns
 */ export function useFhirPagination(url, pageSize, options = {}) {
    return useServerPagination(url, pageSize, getFhirServerPaginationHandlers(), options);
}
export function getFhirServerPaginationHandlers() {
    return {
        getPaginatedUrl: (url, limit, startIndex)=>{
            if (url) {
        ...
        getData: (response)=>response?.entry?.map((entry)=>entry.resource)
    };
}
