...
const runner = (0, node_path_1.resolve)(__dirname, `runner.js`);
...
yargs_1.default.command('debug', 'Starts a new debugging session of the OpenMRS app shell. This uses Webpack as a debug server with proxy middleware.', (argv) => argv
    ...
    describe: 'The import map to use. Can be a path to a valid import map to be taken literally, an URL, or a fixed JSON object.',
    type: 'string',
})
    .option('routes', {
    default: 'routes.registry.json',
    describe: 'The routes.registry.json file to use. Can be a path to a valid routes registry to be taken literally, an URL, or a fixed JSON object.',
    type: 'string',
})
    ...
    if (args.port === undefined) {
        port = await (0, utils_1.getAvailablePort)(8080);
    }
    else {
        // handle case where user has specified a port to run on
        if (!(await (0, utils_1.isPortAvailable)(args.port))) {
    ...
    runCommand('runDebug', {
        ...
        ...args,
        port,
        ...(0, utils_1.proxyImportmapAndRoutes)(await (0, utils_1.mergeImportmapAndRoutes)(await (0, utils_1.getImportmapAndRoutes)(args.importmap, args.routes, port), await (0, utils_1.runProject)(port, args.sources)), args.backend, args.spaPath),
    });
});
yargs_1.default.command('develop', 'Starts a new frontend module development session with the OpenMRS app shell.', (argv) => argv
    ...
    runCommand('runDevelop', {
        ...
        ...args,
        port,
        ...(0, utils_1.proxyImportmapAndRoutes)(await (0, utils_1.mergeImportmapAndRoutes)(await (0, utils_1.getImportmapAndRoutes)(args.importmap, args.routes, port), await (0, utils_1.runProject)(port, args.sources, args['use-rspack']), args.backend, args.spaPath), args.backend, args.spaPath),
    });
});
yargs_1.default.command('build', 'Builds a new app shell.', (argv) => argv
    ...
    type: 'string',
    coerce: (arg) => (0, node_path_1.resolve)(process.cwd(), arg),
})
    .option('registry', {
    alias: 'reg',
